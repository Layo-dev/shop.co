...
const SettingsPage = () => {
  ...
  const [lastProfile, setLastProfile] = useState<ProfileFormValues>({ fullName: "", email: "", phone: "", gender: undefined });

  const profileForm = useForm<ProfileFormValues>({
    resolver: zodResolver(profileSchema),
    defaultValues: { fullName: "", email: "", phone: "", gender: undefined },
    mode: "onSubmit",
  });

  const addressForm = useForm<AddressFormValues>({
    resolver: zodResolver(addressSchema),
    defaultValues: { address_line: "", city: "", state: "", postal_code: "", country: "", is_default: false },
    mode: "onSubmit",
  });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data, error } = await supabase
        .from("profiles")
        .select("first_name, last_name, phone, avatar_url, user_id")
        .eq("user_id", user.id)
        .single();
      if (!error) {
      ...
      const { data: addrRows } = await (supabase as any)
        .from("addresses")
  ...
  const saveProfile = async (values: ProfileFormValues) => {
    ...
    if (error) {
      ...
      const saved: ProfileFormValues = { fullName: values.fullName, email: values.email, phone: values.phone, gender: values.gender };
  ...
  const openAddAddress = () => {
    ...
    addressForm.reset({ address_line: "", city: "", state: "", postal_code: "", country: "", is_default: false });
  ...
  const deleteAddress = async (id: string) => {
    if (!user) return;
    const { error } = await (supabase as any).from("addresses").delete().eq("id", id).eq("user_id", user.id);
    if (error) {
  ...
  const setDefaultAddress = async (id: string) => {
    if (!user) return;
    await (supabase as any).from("addresses").update({ is_default: false }).eq("user_id", user.id);
    await (supabase as any).from("addresses").update({ is_default: true }).eq("id", id).eq("user_id", user.id);
    const { data: addrRows } = await (supabase as any)
      .from("addresses")
      .select("id, user_id, address_line, city, state, postal_code, country, is_default")
      .eq("user_id", user.id)
          ...
          <Card className="glass-card">
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center gap-4">
                ...
                <Input type="file" accept="image/*" onChange={onUploadAvatar} disabled={!isEditing || avatarUploading} />
              </div>
              ...
              <Form {...profileForm}>
                <form onSubmit={profileForm.handleSubmit(saveProfile)} className="space-y-4">
                  ...
                  <div className="flex justify-end gap-2">
                    {!isEditing ? (
                      <Button type="button" className="glass-button" onClick={() => setIsEditing(true)}>Edit</Button>
                    ) : (
                      <>
                        <Button type="button" variant="outline" onClick={() => { profileForm.reset(lastProfile); setIsEditing(false); }}>Cancel</Button>
          ...
          <Card className="glass-card">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {addresses.map((addr) => (
                  <div key={addr.id} className="p-4 rounded-lg border flex items-start justify-between">
                    <div>
